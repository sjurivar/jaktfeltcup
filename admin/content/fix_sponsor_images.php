...
/**
 * Fix Sponsor Images
 * Rename sponsor images to avoid ad-blocker issues
 */
...
try {
    ...
    foreach ($sponsors as $sponsor) {
        ...
        if (!empty($sponsor['logo_filename'])) {
            ...
            $newFilename = 'sponsor_' . $sponsor['id'] . '_' . strtolower(str_replace(' ', '_', $sponsor['name'])) . '.png';
            $newPath = __DIR__ . '/../../src/Bilder/sponsorer/' . $newFilename;
            
            echo "<p>Old filename: " . htmlspecialchars($sponsor['logo_filename']) . "</p>";
            echo "<p>New filename: " . htmlspecialchars($newFilename) . "</p>";
            
            if (file_exists($oldPath)) {
                if (rename($oldPath, $newPath)) {
                    // Update database
                    $newUrl = '/jaktfeltcup/src/Bilder/sponsorer/' . $newFilename;
                    $database->execute(
                        "UPDATE jaktfelt_sponsors SET logo_filename = ?, logo_url = ? WHERE id = ?",
                        [$newFilename, $newUrl, $sponsor['id']]
                    );
                    echo "<p>✅ Successfully renamed and updated database</p>";
    ...
    echo "<h2>Summary</h2>";
    echo "<p>Sponsor images have been renamed to avoid ad-blocker issues.</p>";
